...
### Semi-supervised **Natural Language Understanding**:
  ...
  ![Example of spacy's NLP](/documents/spacy-viz.svg)
...
![New Timeline](documents\timeline%20(2).svg)

### Milestones achieved

|     | Date       | Miletstone Achieved                                                                                         |
| --- | ---------- | ----------------------------------------------------------------------------------------------------------- |
| 1   | 09/22/2019 | Finished the Learning phase as per above gantt diagram                                                      |
| 2   | 09/30/2019 | Data preparation done with a few bugs left to iron out (see `helpers/json_to_csv.py`, `helpers/analyze.py`) |
| 3   | 11/10/2019 | Code for generating answers from the base, untrained version of the model written. Also wrote code for baseline evaluation. (see `first_attempt.ipynb`) |

What `json_to_csv.py` does is that it transform out dataset which is basically 7 giant blobs of JSON objects into CSV files which are easier to work with for our usecase.

The CSV files produced by `json_to_csv.py` are then fed into `analyze.py` which is what converts those CSV data files into natural language corpuses to which would eventually be used to train the model.

You can find the corpus generated by these files in `data/train.txt.` and `data/test.txt`. 

What `first_attempt.ipynb` does is that it downloads a pretrained version of RoBERTa, hereafter referred to as the base model, and provides the prompt to the base model and stores the generated answer. This is used to evaluate the performance on the task of the base model, to be later compared to the trained model to see how much of an improvement it ends up being.

I will also be able to reuse this exact code to debug and evaluate the final model, by simply pointing the transformer model to my learned weights instead of the weights of the base model.

I had also written some code to train the model itself and attempted to run it on MAMBA, but then later decided to switch to using the excellent `transformers` library by huggingface, and trashed most of that code. `transformers` is just much nicer to use, and in the future it would be easy for me to compare different transformer models, should my curiosity compel me to do so. 
